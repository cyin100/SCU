 ...
 *		to by SP. Utilizes quicksort function to sort elements
 ...
 * Function:    quickSort
 *
 * Description:	Recursive helper function used to sort array - finds partition
 *      index and calls itself with left and right sides
 * Complexity: Avg O(nlogn), Worst O(n^2)
 */
static void quickSort(void **elts, int low, int high, int (*compare)()){
    if (low < high){
        //find partition index and recursive quick sort left and right side
        int pi = partition(elts, low, high, compare);
        quickSort(elts, low, pi-1, compare);
        quickSort(elts, pi+1, high, compare);
    }
}
 ...
 * Function:    partition
 *
 * Description:	Returns index that partition should be and ensures everything to
 *      left of pivot is smaller and right is larger
 * Complexity: O(n)
 */
static int partition(void **elts, int low, int high, int (*compare)()){
	...
	int pi = low;
	int i;
    //swap current element with pivot and increment if out of order
	for(i=low; i<high; i++){
