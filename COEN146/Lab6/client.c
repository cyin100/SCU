...
int main(int argc, char *argv[])
{
    ...
    serverAddr.sin_port = atoi(argv[1]); // port
    inet_pton(AF_INET, argv[2], &(serverAddr.sin_addr)); // IP address
    ...
    // send file content to server in packets
    ...
    // create empty packet and send it to server
    char empty[0]; 
    my_send(empty, 0);
...
void my_send(char *data, int nbytes)
{
    ...
    buf.header.checksum = 0; // default checksum value is 0
    if (data != NULL)
    {
    ...
    if (r <= 8)
    {
        sendto(sock, &buf, sizeof(PACKET), 0, (struct sockaddr *)&serverAddr, sizeof(serverAddr));
        printf("Packet sent!\n");
    }
    else
    {
...
void recv_ack(PACKET *buf, int nbytes)
{
    ...
    while (1)
    {
        while (1)
        {
            ...
            // int select(int nfds, fd_se    // STUDENT WORKt *readfds, fd_set *writefds, fd_set *exceptfds, struct timeval *timeout);
            ...
            else if (rv == 0)
            {
                ...
                my_send(buf->data, nbytes); // re-send data
            }
        }
        ...
        int cs_calc = calc_checksum((char *)&receive_buf, sizeof(HEADER));

        // a bad ack has been received
        // resend packet
        // simulating erroneous channel...corruption and loss
        if (cs != cs_calc || receive_buf.seq_ack != state)
        {
            printf("Invalid checksum or invalid sequence number!\n");
