...
#pragma mark - Toolkit Function Implementation
    ...
    // to a copy of the removed item.
    {
        ...
        if (root_ptr->right( ) != NULL)
            ...
            // Update root_ptr then delete and save data to removed
...
#pragma mark - Member Function Implementation
    ...
    // Header file used: bintree.h
    {
        // If self referencing return, else clear and copy 
    ...
    {
        // Use insert all to add addend elements
    ...
    bag<Item> operator +(const bag<Item>& b1, const bag<Item>& b2)
    {
        // Copy b1 and use += operator for b2
        bag<Item> b3 = b1;
        b3 += b2;
        return b3;
    }
    ...
    // method.
    {
        if (addroot_ptr != NULL)
        {
            // Insert data and insert all left and right branches
